...
def process_single(room, sample_rate, novel_path_config, channel_type, results_dir, source1_path, source2_path, source3_path, noise_path, music_path, transcripts):
    """
    Save:
    ...
    spks_nav_points = [ss_utils.get_nav_idx(scene, distance_threshold=5.0) for _ in range(3)]
    spks_nav_mid_points = [spks_nav_points[i][len(spks_nav_points[i]) // 2] for i in range(3)]
    mic_points = ss_utils.get_nav_point_from_grid_points(scene, spks_nav_mid_points, distance_threshold=5.0, num_points=1)[0]
    noise_music_points = ss_utils.get_nav_point_from_grid_points(scene, spks_nav_mid_points, distance_threshold=8.0, num_points=2)
    print(mic_points)
    ...
    source3_audio, start_end_points3, audioname3 = audio_utils.create_long_audio(source3_path, 60)
    ...
    receiver_audio_3 = dynamic_utils.interpolate_moving_audio(source3_audio, ir3_list, spks_nav_points[2])
    ...
    receiver_audio_3 = audio_utils.get_lufs_norm_audio(receiver_audio_3.transpose(0,1).numpy(), sample_rate, -17)[0]
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler(f'generate_mp3d_librispeech_data_test.log'), logging.StreamHandler()])
    ...
    for mode in ["test"]:
        ...
        for idx, scene in enumerate(scene_list):
            ...
            while len(speech_list) > 3:
                ...
                results_dir = f'LibriSpace/{mode}/{scene}/{source1_path.split("/")[-1].split(".")[0]}-{source2_path.split("/")[-1].split(".")[0]}-{source3_path.split("/")[-1].split(".")[0]}'
