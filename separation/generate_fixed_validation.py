...
def generate_fixed_validation(raw_dir, save_dir, noise_type="noise", n_src=2, num_samples=300, is_mono=False):
    ...
    for idx in tqdm(range(len(test_datasets))):
        ...
        for spk in range(n_src):
            torchaudio.save(os.path.join(save_dir, f"sample{sample_idx}", "s1.wav"), speaker_wav[spk:spk+1,...] if speaker_wav.dim() == 2 else speaker_wav[spk], 16000)
        sample_idx += 1
