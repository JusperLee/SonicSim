...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        if x.dim() > 3:
            raise RuntimeError("Encoder accept 1/2D tensor as input, but got {:d}".format(x.dim()))
...
class ConvTasNet(BaseModel):
    ...
    def forward(self, x):
        ...
        # w: n x B x T
        m = self.mask(w)
        # m: n x H*num_spks x T
        m = torch.chunk(m, chunks=self.num_spks, dim=1)
        # m: [n x H x T]
        m = self.non_linear(torch.stack(m, dim=0))
        # m: num_spks x n x H x T
        d = [x*m[i] for i in range(self.num_spks)]
        # d: [n x H x T]
        s = [self.pad2(self.decoder(d[i]), nsample) for i in range(self.num_spks)]
        # s: [n x T]
        return torch.stack(s, dim=1) if x.shape[0] != 1 else torch.stack(s, dim=0)
