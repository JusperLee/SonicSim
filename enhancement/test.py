...
def test(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for idx in range(len(test_datasets)):
        ...
        with torch.no_grad():
            ...
            json_dicts = {}
            with open(f"../SonicSim-SonicSet/SonicSet/scene_datasets/mp3d/test/test/{folder}/{filename}/json_data.json", "r") as f:
            ...
            for start, end in speaker_start_end:
                ...
                # ehn_out = TaylorWav(ehn_out, spks_tmp, cfg.loss.n_fft, cfg.loss.hop_length, cfg.loss.win_length)
