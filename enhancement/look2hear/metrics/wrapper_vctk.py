...
class MetricsTrackerVCTK:
    def __init__(self, save_file: str = ""):
        ...
        csv_columns = ["snt_id", "sdr", "sdr_i", "si-snr", "si-snr_i", 'pesq_wb', 'stoi', "csig", "cbak", "covl"]
    ...
    def __call__(self, mix, clean, estimate, key):
        ...
        # PESQ
        pesq_wb = compute_pesq(clean.squeeze(0).cpu().numpy(), estimate.squeeze(0).cpu().numpy(), 16000)
        ...
        # # CSIG
        # csig = compute_csig(clean.squeeze(0).cpu().numpy(), estimate.squeeze(0).cpu().numpy(), 16000)
        
        # # CBAK
        # cbak = compute_cbak(clean.squeeze(0).cpu().numpy(), estimate.squeeze(0).cpu().numpy(), 16000)
        
        # # COVL
        # covl = compute_covl(clean.squeeze(0).cpu().numpy(), estimate.squeeze(0).cpu().numpy(), 16000)
        # import pdb; pdb.set_trace()
        dns_dict = self.dnsmos(estimate.squeeze(0).cpu().numpy(), 16000, True)
        ...
        self.all_pesq_wb.append(pesq_wb)
        self.all_stoi.append(stoi.cpu().item())
        self.all_csig.append(dns_dict["SIG"])
        self.all_cbak.append(dns_dict["BAK"])
        self.all_covl.append(dns_dict["OVRL"])
    ...
    def update(self, ):
        return {"sdr": np.array(self.all_sdrs).mean(),
                ...
                "csig": np.array(self.all_csig).mean(),
                "cbak": np.array(self.all_cbak).mean(),
                "covl": np.array(self.all_covl).mean(),
                }
    ...
    def final(self,):
        ...
        row = {
            ...
            "csig": np.array(self.all_csig).std(),
            "cbak": np.array(self.all_cbak).std(),
            "covl": np.array(self.all_covl).std(),
        }
        self.writer.writerow(row)
