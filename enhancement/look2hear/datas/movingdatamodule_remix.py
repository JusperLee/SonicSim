...
class MovingTrainDataset(Dataset):
    def __init__(
        ...
        # json_dict
        self.json_start_end = {}
        with open("./tests/segment-train.json", 'r') as f:
            self.json_start_end = json.load(f)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        speech_dir_key = random.choice(list(self.json_start_end.keys()))
        speaker_wavs = []
        speaker_id = [int(i) for i in speech_dir_key.split("/")[-1].split("-")]
        speech_dir = speech_dir_key[:-4]
        speaker_id.sort()
        speaker_id = random.choices(speaker_id, k=1)
        for index, idx in enumerate(speaker_id):
        ...
        for noise in noise_types:
            ...
            if noise == "noise":
                noise_wav = overlap_audio(noise_wav.view(1, -1), self.sample_rate, delay=6).view(-1)
            noise_wavs.append(noise_wav)
        noise_wav = torch.stack(noise_wavs)
        
            
         # 随机选择时间范围
        start_end = self.json_start_end[speech_dir_key]
        start_end = random.choice(start_end)
        start, end = start_end
        ...
        return mix_wav, speaker_wav.mean(0)#, all_noise
...
class MovingTestEvalDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        speaker_wavs, _ = torchaudio.load(os.path.join(folder, 's{}.wav'.format(self.num_spks+1)))
        if self.is_mono:
