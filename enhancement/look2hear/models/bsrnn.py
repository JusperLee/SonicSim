...
class BSNet(nn.Module):
    def __init__(self, in_channel, nband=7):
        ...
        self.band_rnn = ResRNN(self.feature_dim, self.feature_dim*2, bidirectional=False)
...
class BSRNN(BaseModel):
    def __init__(self, sample_rate=44100, win=2048, stride=512, feature_dim=16, num_repeat=6, num_output=1):
        ...
        bandwidth_200 = int(np.floor(100 / (sample_rate / 2.) * self.enc_dim))
        bandwidth_500 = int(np.floor(250 / (sample_rate / 2.) * self.enc_dim))
        bandwidth_2k = int(np.floor(1000 / (sample_rate / 2.) * self.enc_dim))
        ...
        self.band_width += [bandwidth_2k]*3
